...
export default function DomeGallery({
  ...
  // --- THE NEW AUTO-ROTATION LOOP ---
  const autoRotateRAF = useRef<number | null>(null);
  useEffect(() => {
    const autoRotateSpeed = 0.05; // Adjust this speed if you want it faster/slower
    const loop = () => {
      // Only spin if the user is NOT dragging, releasing inertia, or opening a modal
      if (
        ...
        (applyTransform as (x: number, y: number) => void)(rotationRef.current.x, rotationRef.current.y);
      }
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.addEventListener('transitionend', safeCleanup, { once: true });
      window.setTimeout(safeCleanup, enlargeTransitionMs + 500);
    };
  ...
  // UI UPGRADE: Added box-shadows to the tiles for a more 3D lighting feel!
  const cssStyles = `
    ...
    .item__image { position: absolute; inset: 10px; border-radius: var(--tile-radius, 12px); overflow: hidden; cursor: pointer; backface-visibility: hidden; -webkit-backface-visibility: hidden; transition: transform 300ms, box-shadow 300ms; pointer-events: auto; -webkit-transform: translateZ(0); transform: translateZ(0); box-shadow: 0 10px 30px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.1); }
    .item__image:hover { box-shadow: 0 10px 40px rgba(255,255,255,0.1), inset 0 0 0 1px rgba(255,255,255,0.3); }
          ...
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  <div className="item__image absolute block overflow-hidden cursor-pointer bg-gray-900" role="button" tabIndex={0} aria-label={it.alt || 'Open image'}
