...
import { useMousePosition } from "../hooks/useMousePosition"; // Ensure this path matches your structure
...
`;
const KEY_DURATIONS = Array.from({ length: 18 }, () => 0.5 + Math.random());
...
export default function CyberPenguin({ isCoding = false }: { isCoding?: boolean }) {
    ...
    // Handle Eye Tracking Math
    useEffect(() => {
        // If mouse is at 0,0 (initial load), keep eyes centered
        if (x === 0 && y === 0) return;

        if (!containerRef.current) return;
        const rect = containerRef.current.getBoundingClientRect();

        // Approximate center of the penguin's face
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height * 0.4;

        const angle = Math.atan2(y - centerY, x - centerX);

        // Limit how far the eyes can move (max 14 pixels)
        const maxDistance = 25;
        const distance = Math.min(maxDistance, Math.hypot(x - centerX, y - centerY) / 60);
        ...
        <div ref={containerRef} className="penguin-container relative flex flex-col items-center justify-center pointer-events-auto">
                            ...
                            <div className="w-[90%] h-[60%] border border-[var(--accent-emerald)]/20 grid grid-cols-6 gap-1 p-1">
                                {[...Array(18)].map((_, i) => (
                                    <motion.div
                                        ...
                                        className="bg-[var(--accent-emerald)]/30 rounded-sm" // Fixed 'rounded-xs' error
                                    />
                                ))}
                            </div>
                        </div>
            ...
            >
                <defs>
                    ...
                    <radialGradient id="eyeG" cx="35%" cy="30%" r="65%">
                        <stop offset="0%" stopColor="#ffffff" />
                        <stop offset="100%" stopColor="#c0e0ff" />
                    </radialGradient>
                    <radialGradient id="chestG" cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stopColor="#10b981" />
                ...
                {/* Left Wing (Typing Animation) */}
                {/* Speed increases heavily when isCoding is true */}
                <motion.g
                    animate={{ rotate: isCoding ? [0, -25, 0] : 0 }}
                    transition={
                        isCoding
                            ? { duration: 0.15, repeat: Infinity, ease: "linear" }
                            // When false, use a stiff spring that completely settles
                            : { type: "spring", stiffness: 400, damping: 30 }
                    }
                    style={{ transformOrigin: "50px 218px" }}
                >
                    <ellipse cx="50" cy="264" rx="20" ry="46" fill="url(#bodyG)" transform="rotate(-10 50 264)" />
                </motion.g>

                {/* Right Wing (Typing Animation) */}
                <motion.g
                    animate={{ rotate: isCoding ? [0, 25, 0] : 0 }}
                    transition={
                        isCoding
                    ...
                    style={{ transformOrigin: "190px 218px" }}
                >
                ...
                {/* Wrapped in motion.g so the eye elements and eyelids move together based on pupilPos */}
                <motion.g animate={{ x: pupilPos.x, y: pupilPos.y }} transition={{ type: "spring", stiffness: 100, damping: 20 }}>
                ...
                <motion.g animate={{ x: pupilPos.x, y: pupilPos.y }} transition={{ type: "spring", stiffness: 150, damping: 20 }}>
